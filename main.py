...
async def on_message(message):
    with open('C:\\Users\\Kiril\\Desktop\\discord-bot\\lvl.json', 'r') as f:
        global users
        users = json.load(f)
    ...
    with open('C:\\Users\\Kiril\\Desktop\\discord-bot\\lvl.json', 'w') as f:
        json.dump(users, f)
...
async def about_me(ctx):
    if ctx.author.nick != None:
        msg = f"**Имя** - {client.get_user(ctx.author.id)}\n" \
              f"**id** - {ctx.author.id}\n" \
              f"**exp** - {users[str(ctx.message.author.id)]['exp']}/100\n" \
